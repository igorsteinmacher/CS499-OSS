...
3. Choose an already existing branch and use `git diff BRANCH_NAME` to view the differences between that branch and the current branch. Summarize the difference between the main and the other branch.
...
4. Write a command sequence to merge the branch that is not the master branch into `main`.
...
5. Write a command (or sequence) to (i) create a new branch called `math` (from the `main`) and (ii) change to this branch.
...
8. Change back to the `master` branch and change B.py, adding the following source code (commit your change to `main`):
```
    def print_hello(this):
        print("hello world!");
```

9. Write a command sequence to merge the `math` branch into `main` and describe what happened.
```
...
12. Write a command (or set of commands) to proceed with the merge and make `main` branch up-to-date.
```
